...
from karel.syntax import analyze
...
def begin_karel_program():
    ...
    if len(sys.argv) >= 2:
        ...
        print("\nWARNING: No world specified, we'll let Karel run in an empty world.")
        print("         (World file to load can be given as first command argument.)")

    # run syntax analysis on the program to ensure it conforms to karel's language
    if _analyze_syntax:
        result = analyze(sys.argv[0])
        # potential syntax problems found
        if not result:
            print("This program does not technically adhere to Karel's language syntax,")
            print("but it may still execute correctly. Proceeding with caution...")
    ...
    if speed >= 1.0:
        ...
        _wait = 1000.0 / (2.0 ** (5.0 * speed))

    # write information message on world and simulation speed
    if len(sys.argv) >= 2:
        print('\nRunning Karel program in', sys.argv[1], 'at speed', speed)
...
def end_karel_program():
    if not _karel.error:
        print('Karel program terminated successfully.')
    if _allow_edit:
        _run_editor_loop()
    else:
...
def _turn_right():
    """
    The turn_right() instruction is technically not part of Karel's instruction
    set, but can be included to make animation speedier for complex programs.
    """
    if not _ready():
        return
...
def _turn_around():
    """
    The turn_around() instruction is likewise not part of Karel's language.
    """
    if not _ready():
        return

    # directions are ordered N, E, S, W, so turn left goes backward in the list
    _karel.facing = (_karel.facing + 2) % 4
    _update()
