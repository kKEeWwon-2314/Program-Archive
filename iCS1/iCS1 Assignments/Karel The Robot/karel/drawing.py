...
# Size of display (pixels) obtained from pygame
_display_width = constants.MAX_WIDTH_PIXELS
_display_height = constants.MAX_HEIGHT_PIXELS
...
def _calculate_cell_size(avenues, streets):
    ...
    """
    cell_width = _display_width / float(avenues + 2)
    cell_height = _display_height / float(streets + 1)
...
def initialize(karel_world):
    ...
    # suggest that the window be opened centred on the desktop
    os.environ['SDL_VIDEO_CENTERED'] = '1'

    # disable UI scaling on high-DPI systems
    if sys.platform.startswith('win'):
        user32 = ctypes.windll.user32
        user32.SetProcessDPIAware()

    # retrieve display size from pygame
    pygame.init()
    display_info = pygame.display.Info()
    _display_width = min(display_info.current_w - 80, constants.MAX_WIDTH_PIXELS)
    _display_height = min(display_info.current_h - 80, constants.MAX_HEIGHT_PIXELS)
    ...
    stddraw.setFontFamily('Helvetica,Arial,DejaVu Sans')
